...
make_feature_list <- function(feature.anno, database, gene.anno=NULL){
	...
	feature <- read.table(feature.anno, sep="\t", col.names=feature.cols, 
		row.names=NULL, stringsAsFactors=FALSE, comment.char="")
...
anno <- list()
feature.anno <- paste('./',species,'.',feature,'.',db.name,'.txt', sep='')
print(feature.anno)
